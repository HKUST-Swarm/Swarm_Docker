...
roslaunch ptgrey_reader stereo.launch is_sync:=false UP_ID:=UP_ID DOWN_ID:=DOWN_ID &> $LOG_PATH/log_camera.txt &
PG_PID=$!
...
then
    ...
    /bin/sleep 5
    sudo kill -- $PG_PID
    roslaunch ptgrey_reader stereo.launch UP_ID:=UP_ID DOWN_ID:=DOWN_ID &> $LOG_PATH/log_camera.txt &
